...
public:
    ...
    // Helper function that maps a number (row or column number)
    // to its corresponding value in the 1D representation
    int convertToIndex(int n) const;

    // Helper function that takes a row, col, and direction
    // and returns a new row, col that matches the directional
    // change
    Location adjustLocation(int row, int col, Direction direction) const;

    // Helper function to convert basic (row, cell) into a new location
    // that is the transformed row, cell
    Location getCellArrayCoord(int cellRow, int cellCol) const;

    // Helper function that converts original (row, col) location
    // into its 1D index
    int getArrayIndex(int row, int col) const;

    // Same as getArrayIndex but instead of original (row, col),
    // this function expects expanded/ transformed coordinates
    // for row and col.
    int getArrayIndexExp(int row, int col) const;
