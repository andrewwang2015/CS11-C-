...
/* Finds first match of regular expression regex in the string s.
...
Range find(vector<RegexOperator *> regex, const string &s)
{
    ...
    {
        Range temp = findAtIndex(regex, s, i);
...
/* Determines if a regex expression matches the entire string */
bool match(vector<RegexOperator *> regex, const string &s)
{
    Range temp = find(regex, s);
    if (temp.start == 0 and temp.end == s.length())
    {
        return true;
    }
    return false;
